...
{
	{
		int first = 0, second = 1, third; //we have three int value in here 

		for (int i = 1; i < fiIndex; i++) // i start from 1 and increasing one by one
		{
			third = first + second;
			first = second;
			second = third;
		}
		return first; //at the end of transactions function go back to first int value
	}
...
char* ce103_strrev(char* fiStr)
{
	...
	char temp;         //we have 4 different value .
	int x, y, n;
	n = ce103_strlen(fiStr); // we assignet n to fiStr.

	for (x = 0, y = n - 1; x < y; ++x, --y)  // here we increase x by one and decrease y by 1 to swap letters,so the word is spelled backwards.
	...
	return fiStr; // when we finish, function come back to fiStr.
...
int ce103_strlen(const char* fiStr)
{
	...
	int c = 0;  // here we assign c to 0

	while (fiStr[c] != '\0') // we start from 0 to find the number of loop letters here

		c++;

	return c; // We continue the loop until we find the number
}
...

{
	...
	char* ret = fiDest;  //first we  assign fiDest to ret
	while (*fiDest++ != '\0'); // if fiDest is not equal to 0 we decrease its value by 1
	--fiDest;
	while ((*fiDest++ = *fiSrc++) != '\0'); // Code continues until fidest and fisrc are synchronized

	return ret; //come back to ret and continue
}
...
{
	//TODO:Start from Here...
	int ret;    //We define an int variable named ret
	while ((ret = *fiLhs - *fiRhs++) == 0 && *fiLhs++); //if the filhs variable and the firhs variable are the same, the result will be 0 and the change will be detected
	return ret; //its come bact to ret
...
char* ce103_strcpy(char* foDestination, const char* fiSource)
{
	...
	char* s1 = foDestination;   //We equate the char s1 variable with the foDestination
	while ((*foDestination++ = *fiSource++) != '\0'); // If foDestination is not equal then the result is no (0) if the fiSource is not equal then the result is yes(1) if it is equal
	return s1; //come back to s1
...
{
	//TODO:Start from Here...
	int i;              // we have int value 
	for (i = 0; i < fiBinLen; i++) {         //The value of i starts from 0 and increases by one as long as fibinlen is greater than i. Executes the function inside the for loop
		foHex[i * 2] = "0123456789ABCDEF"[fiBin[i] >> 4];
		foHex[i * 2 + 1] = "0123456789ABCDEF"[fiBin[i] & 0x0F]; // We changed fohex value one by one and then we converting bin values to hex
	}
	foHex[fiBinLen * 2] = '\0'; // If the function value is equal to 0, the operation is restarted.
